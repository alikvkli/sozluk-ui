...
import NotFoundEntry from "@/components/pages/entry/NotFoundEnry";
...
export default function EntryPage(props: EntryDetailProps) {
    ...
    return (
        <>
            <Head>
                <title>{props.entry.payload.length > 0 ? `"#${props.entry?.payload[0]?.entry_id}" numaralı entry hakkında |` : ""}{brandName} </title>
                <meta name="description" content="Generated by create next app" />
                <meta name="viewport" content="width=device-width, initial-scale=1" />
                <link rel="icon" href="/favicon.ico" />
            </Head>
            <MainLayout>
                {props.entry.payload.length > 0 && <EntryDetail />}
                {props.entry.payload.length === 0 && <NotFoundEntry />}
            </MainLayout>
        </>
    )
}
